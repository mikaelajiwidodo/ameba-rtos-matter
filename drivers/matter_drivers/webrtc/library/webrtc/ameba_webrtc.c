...
static void ice_on_candidate(void *ctx, ameba_ice_candidate_t *candidate)
{
    ...
    if (session == NULL || candidate == NULL) return;
    ...
    const ameba_webrtc_ice_server_t *ice_servers, int num_servers)
{
    ...
    if (session->ice == NULL) goto error;
    ...
    if (session->dtls == NULL) goto error;
...
error:
    if (session->ice) ameba_ice_destroy(session->ice);
    if (session->dtls) ameba_dtls_destroy(session->dtls);
    if (session->dc_mgr) ameba_datachannel_mgr_destroy(session->dc_mgr);
    free(session);
    return NULL;
}
...
void ameba_webrtc_session_destroy(ameba_webrtc_session_t *session)
{
    ...
    if (session->udp_fd >= 0) lwip_close(session->udp_fd);
...
{
    if (session == NULL || ip == NULL) return -1;
...
{
    if (session == NULL || session->ice == NULL) return -1;
    ...
    ameba_webrtc_session_t *session, const char *sdp, int sdp_type)
{
    ...
    SESS_DBG("set_remote_description: parsed SDP, ice_ufrag='%s', ice_pwd='%s'",
             session->sdp_info.ice_ufrag, session->sdp_info.ice_pwd);
    ...
    /* Add remote ICE candidates from SDP */
    int i;
    for (i = 0; i < session->sdp_info.media_count; i++) {
        /* Add candidate from connection info in SDP */
    ...
    ameba_webrtc_session_t *session, const char *candidate, const char *mid)
{
    ...
    if (sscanf(candidate, "%*s %*u %*s %*d %63s %d typ %*s", ip, &port) >= 2) {
        ...
        if (mid && strstr(mid, "rtcp")) component_id = 2;
                                           ...
                                           const char *mid, int payload_type)
{
    ...
    if (session->track_count >= AMEBA_WEBRTC_MAX_TRACKS) return -1;
                                      ...
                                      uint32_t timestamp)
{
    ...
    if (!session->remote_addr_set || !session->on_send_udp) return -1;

    for (i = 0; i < session->track_count; i++) {
        if (session->tracks[i].type != AMEBA_WEBRTC_TRACK_VIDEO ||
            !session->tracks[i].active) continue;
        ...
        while (rem > 0) {
            ...
            if (ameba_h264_find_nal(pos, rem, &nal_off, &nal_len) != 0) break;
                ...
                if (nalu_len <= max_payload) {
                    ...
                    while (frag_rem > 0) {
                        ...
                        if (plen > 0) {
                            ...
                            if (end && is_last_nal) rtp_pkt[1] |=  0x80;
                            else                    rtp_pkt[1] &= ~0x80;
                            ameba_webrtc_send_rtp_pkt(session, i, rtp_pkt, (size_t)plen);
                        }
                        frag     += chunk;
                                      ...
                                      uint32_t timestamp)
{
    ...
    if (session->state < AMEBA_WEBRTC_STATE_CONNECTED) return -1;
...
void ameba_webrtc_session_close(ameba_webrtc_session_t *session)
{
    ...
    if (session->dtls) ameba_dtls_close(session->dtls);
    if (session->ice) ameba_ice_stop(session->ice);
...
{
    if (session->ice) ameba_ice_tick(session->ice);
    if (session->dtls) ameba_dtls_tick(session->dtls);
}
