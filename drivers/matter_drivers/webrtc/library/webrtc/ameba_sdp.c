...
int ameba_sdp_parse(const char *sdp, ameba_sdp_info_t *info)
{
    ...
    if (sdp == NULL || info == NULL) return -1;
    ...
    while (*ptr) {
        ...
        if (nl) {
            ...
            if (line_len > AMEBA_SDP_MAX_LINE - 1) line_len = AMEBA_SDP_MAX_LINE - 1;
        ...
        case 'm': {
            media_idx++;
            if (media_idx >= AMEBA_SDP_MAX_MEDIA) break;
...
int ameba_sdp_generate_offer(const ameba_sdp_info_t *info, char *buf, size_t buf_size)
{
    ...
    for (i = 0; i < info->media_count; i++) {
        ...
        for (j = 0; j < m->payload_count; j++) {
            ...
            if ((size_t)n + tmp >= remaining) return -1;
        ...
        switch (m->direction) {
        case AMEBA_SDP_DIR_SENDONLY: dir_str = "sendonly"; break;
        case AMEBA_SDP_DIR_RECVONLY: dir_str = "recvonly"; break;
        case AMEBA_SDP_DIR_INACTIVE: dir_str = "inactive"; break;
        default: dir_str = "sendrecv"; break;
        }
                     ...
                     const char *fingerprint)
{
    ...
    if (ice_ufrag) strncpy(info->ice_ufrag, ice_ufrag, sizeof(info->ice_ufrag) - 1);
    if (ice_pwd) strncpy(info->ice_pwd, ice_pwd, sizeof(info->ice_pwd) - 1);
    if (fingerprint) {
                         ...
                         const char *mid)
{
    ...
    if (info->media_count >= AMEBA_SDP_MAX_MEDIA) return -1;
    ...
    if (codec) strncpy(m->codec, codec, sizeof(m->codec) - 1);
