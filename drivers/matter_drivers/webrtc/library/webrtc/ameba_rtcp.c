...
{
    if (data == NULL || len < sizeof(ameba_rtcp_nack_t)) return 0;

    /* Check for RTPFB with FMT=1 (NACK) */
    if (data[0] != (2 << 6 | 1)) return 0; /* V=2, FMT=1 */
    if (data[1] != AMEBA_RTCP_RTPFB) return 0;

    /* Extract first NACK pair */
    ameba_rtcp_nack_pair_t *pair = (ameba_rtcp_nack_pair_t *)(data + sizeof(ameba_rtcp_nack_t));
    if ((size_t)((uint8_t *)(pair + 1) - data) > len) return 0;
