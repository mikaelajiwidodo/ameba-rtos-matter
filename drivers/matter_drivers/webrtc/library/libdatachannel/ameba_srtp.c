                     ...
                     uint32_t ssrc)
{
    ...
    for (int _i = 0; _i < 16; _i++) SRTP_DBG("%02X", master_key[_i]);
    SRTP_DBG("\n");
    SRTP_DBG("[SRTP-DBG]   master_salt: ");
    for (int _i = 0; _i < 14; _i++) SRTP_DBG("%02X", master_salt[_i]);
    SRTP_DBG("\n");
    SRTP_DBG("[SRTP-DBG]   session_encr_key: ");
    for (int _i = 0; _i < 16; _i++) SRTP_DBG("%02X", ctx->session_encr_key[_i]);
    SRTP_DBG("\n");
    SRTP_DBG("[SRTP-DBG]   session_salt: ");
    for (int _i = 0; _i < 14; _i++) SRTP_DBG("%02X", ctx->session_salt[_i]);
    SRTP_DBG("\n");
    SRTP_DBG("[SRTP-DBG]   session_auth_key: ");
    for (int _i = 0; _i < 20; _i++) SRTP_DBG("%02X", ctx->session_auth_key[_i]);
    SRTP_DBG("\n");
}
                       ...
                       uint8_t *out_buf, size_t *out_len)
{
    ...
    *out_len = rtp_len + AMEBA_SRTP_AUTH_TAG_LEN;
    ...
    if (seq < 3) {
        ...
        for (int _i = 0; _i < 16; _i++) SRTP_DBG("%02X", iv[_i]);
        SRTP_DBG("\n");
        ...
        for (size_t _i = 0; _i < *out_len; _i++) SRTP_DBG("%02X", out_buf[_i]);
        SRTP_DBG("\n");
    }
