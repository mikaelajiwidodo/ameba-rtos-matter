                               ...
                               const uint8_t *data, size_t len)
{
    ...
    case AMEBA_DC_MSG_OPEN: {
        ...
        if (len < sizeof(ameba_dc_open_t)) return -1;

        if (mgr->channel_count >= AMEBA_DC_MAX_CHANNELS) return -1;
    ...
    case AMEBA_DC_MSG_DATA_UNRELIABLE: {
        ...
        if (len < 3) return -1; /* type(1) + stream_id(2) */
                            ...
                            const uint8_t *data, size_t len)
{
    ...
    for (i = 0; i < mgr->channel_count; i++) {
        if (mgr->channels[i].id == channel_id) {
            if (!mgr->channels[i].open) return -1;
