...
public:
    ...
    void CreateOffer() override
    {
        ...
        if (mSession == NULL) { ChipLogError(Camera, "WebRTC session not created"); return; }
    ...
    void SetRemoteDescription(const std::string & sdp, SDPType type) override
    {
        ...
        switch (type)
        {
        case SDPType::Offer:    sdp_type = 0; break;
        case SDPType::Answer:   sdp_type = 1; break;
        case SDPType::Pranswer: sdp_type = 2; break;
        case SDPType::Rollback: sdp_type = 3; break;
        }
    ...
    std::shared_ptr<WebRTCTrack> AddTrack(MediaType mediaType, const std::string & mid, int payloadType) override
    {
        ...
        if (mSession == NULL) return nullptr;
        ...
        if (trackId < 0) { ChipLogError(Camera, "Failed to add track"); return nullptr; }
...
static void OnLocalDescriptionBridge(void *ctx, const char *sdp, int sdp_type)
{
    ...
    if (self && self->mOnLocalDescriptionCB)
    {
        ...
        if (sdp_type == 1) type = SDPType::Answer;
        else if (sdp_type == 2) type = SDPType::Pranswer;
        else if (sdp_type == 3) type = SDPType::Rollback;
